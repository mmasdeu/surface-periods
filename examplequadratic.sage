...
# A good curve
# over Q(i), conductor (22 + 15*i)
x = QQ['x'].gen()
F.<r> = QuadraticField(-1)
NA = F.ideal(22 + 15 * r)
Crv = HyperellipticCurve(x^6 + (8+2*r)*x^5+(25+10*r)*x^4+(40+14*r)*x^3+36*x^2+(14-10*r)*x+(1-4*r))
...
Binvs = quaternion_algebra_invariants_from_ramification(F,D)
G = BigArithGroup(p,Binvs,F.ideal(1),base = F,grouptype = 'PGL2')
Coh = CohomologyGroup(G.Gpn)
V = F.primes_of_bounded_norm(100)
q = V[3]
...
wp = G.wp()
# pg = p.gens_reduced()[0]
# G.wp = lambda:G.Gn.B([pg/2,(-1-pg)/2,-pg/2,(pg-1)/2])
